...
theorem Equiv.Perm.ToPequiv.toMatrix_mem_unitaryGroup {n : Type _} [DecidableEq n] [Fintype n]
  ...
  by
  simp_rw [Matrix.mem_unitaryGroup_iff, ← Matrix.ext_iff, Matrix.hMul_eq_hMul, Matrix.mul_apply,
    PEquiv.toMatrix_apply, boole_mul, Equiv.toPEquiv_apply, Matrix.one_apply, Option.mem_def,
    Matrix.star_apply, PEquiv.toMatrix_apply, star_ite, star_one, star_zero, Finset.sum_ite_eq,
    Finset.mem_univ, if_true]
  intro i j
  simp_rw [Equiv.toPEquiv_apply, Option.mem_def, eq_comm,
    Function.Injective.eq_iff (Equiv.injective _)]
