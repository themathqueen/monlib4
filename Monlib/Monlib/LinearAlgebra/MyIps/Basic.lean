...
theorem ContinuousLinearMap.IsSelfAdjoint.ext_iff_inner_map {E 𝕜 : Type _} [IsROrC 𝕜]
  ...
  by
  rw [← sub_eq_zero, ← is_self_adjoint.inner_map_self_eq_zero (IsSelfAdjoint.sub hp hq)]
...
theorem isSelfAdjoint_iff_complex_inner_re_eq [InnerProductSpace ℂ E] [CompleteSpace E]
  ...
  simp_rw [re_to_complex, ← Complex.conj_eq_iff_re, inner_conj_symm, is_self_adjoint_iff',
    ContinuousLinearMap.ext_iff_inner_map a.adjoint a, adjoint_inner_left]
...
theorem IsSelfAdjoint.adjoint [InnerProductSpace 𝕜 E] [CompleteSpace E] {a : E →L[𝕜] E}
    (ha : IsSelfAdjoint a) : IsSelfAdjoint a.adjoint :=
  congr_arg star ha
...
theorem ContinuousLinearMap.inner_map_self_eq_zero [InnerProductSpace ℂ E] {p : E →L[ℂ] E} :
  ...
  simp_rw [ContinuousLinearMap.ext_iff, ← ContinuousLinearMap.coe_coe, ←
    ContinuousLinearMap.toLinearMap_eq_coe, ← LinearMap.ext_iff]
...
theorem ContinuousLinearMap.adjoint_smul {K E : Type _} [IsROrC K] [NormedAddCommGroup E]
    ...
    (a • φ).adjoint = starRingEnd K a • φ.adjoint := by
...
theorem LinearMap.adjoint_smul {K E : Type _} [IsROrC K] [NormedAddCommGroup E]
    ...
    (a • φ).adjoint = starRingEnd K a • φ.adjoint :=
  by
  have :=
...
theorem LinearMap.adjoint_one {K E : Type _} [IsROrC K] [NormedAddCommGroup E]
    [InnerProductSpace K E] [FiniteDimensional K E] : (1 : E →ₗ[K] E).adjoint = 1 :=
  star_one _
