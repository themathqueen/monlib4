# from https://github.com/YaelDillies/LeanAPAP/blob/master/scripts/count_sorry.py
...
def pretty_file(file: str):
    ...
    url = f"https://github.com/themathqueen/Monlib4/blob/master/{file}"
    return f"[{name}]({url})"

files = [file for tree in os.walk("Monlib") for file in glob(os.path.join(tree[0], '*.lean'))]
sorries = {}
